...
### 新增
- **顯示相容模式（防呆）開關**，三種：
  - **自動（預設）**：前台以 JS 偵測浮動按鈕是否被佈景主題影響（染色／變形／消失），
    偵測到才強制套用你設定的顏色與尺寸。友善主題完全不加 `!important`，保持乾淨。
  - **一律強制**：永遠以最高優先級（`!important`）套用，適合已知主題很強勢的站。
  - **關閉**：不強制，方便自行用 CSS 微調外觀。
- v1.1.5 的無條件 `!important` 改為受此開關控制，避免對友善主題過度強制。

## 1.1.5 — 2026-07-07

### 修正
- 主按鈕與 App 圖示的關鍵樣式（圓形底色、尺寸、圖示顏色）改為 inline 輸出，
  避免強勢主題的按鈕樣式覆蓋，導致按鈕變透明、縮小、圖示顏色被主題繼承
  （例如某些主題把浮動按鈕染成主題色而破圖）。
- SVG 圖示補上明確尺寸屬性，開／關 icon 交換改由 JS 控制，
  即使外部 CSS 未套用也能正確顯示。

## 1.1.4 — 2026-07-07

### 調整
- QR Code 卡片改為「widget 同側角落」彈出（與傳統 click-to-chat 外掛
  的彈窗位置一致），不再全螢幕置中遮罩。
- QR 卡片樣式全部改由 JS inline 輸出，不依賴 CSS 檔（避免 CDN 快取舊樣式）。
