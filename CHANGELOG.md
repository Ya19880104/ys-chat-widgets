...
### 新增
- **對齊位置** 三選項：靠左下 / 靠中下 / 靠右下（原本只有左右）。
- **大小可分別調整**：主按鈕大小（外）與聯絡圖示大小（內）兩個設定，
  圖示會依比例縮放。預設維持原本的 56 / 46。

## 1.2.0 — 2026-07-07

### 新增
- **每個聯絡方式都可上傳自訂圖示**（不再只有內建圖示）；留空則用內建圖示。
- **每個聯絡方式都可自訂顯示名稱**（原本只有「自訂連結」能改）；留空用預設名稱。
- **標籤文字可關閉**：標籤文字新增「不顯示文字（只有圖示）」。
- 新增內建 **Instagram**（正確圖示與網址，帳號或完整網址皆可）。
- NinjaTeam 移轉：自訂 App 的圖示（url-icon）一併移轉為自訂圖示。
